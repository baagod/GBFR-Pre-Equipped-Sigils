...
/** Player overlay for one character: true = enabled (default true when absent). */
type ExclusiveState = { t1: boolean; t2: boolean; war: boolean }
...
export default function App() {
  ...
  const [exclusiveState, setExclusiveState] = useState<Record<string, ExclusiveState> | undefined>(undefined)
  ...
  // Exclusive toggle update (per character, three bits).
  const updateExclusive = (hash: string, key: keyof ExclusiveState, value: boolean) => {
    setExclusiveState((prev) => {
      const current = prev ? { ...prev } : {}
      const entry = current[hash] ?? { t1: true, t2: true, war: true }
      current[hash] = { ...entry, [key]: value }
      return current
    })
  }
  ...
  return (
    <div className="fixed inset-0 flex flex-col">
      <div className="flex shrink-0 items-center gap-1 border-b bg-background px-4 pt-2">
        <Button
        ...
        <Button
          variant={tab === "exclusive" ? "secondary" : "ghost"}
          size="sm"
...
function ExclusivePanel({
  ...
  table: Exclusive[]
  state: Record<string, ExclusiveState> | undefined
  sigilByGem: Map<string, Sigil>
  traitByName: Map<string, Trait>
  lang: Lang
  onChange: (hash: string, key: keyof ExclusiveState, value: boolean) => void
}) {
  ...
  const traitName = (hash: string) => {
    const tr = traitByName.get(hash)
    if (!tr) return hash
  ...
  return (
    <div className="space-y-1">
      {table.map((e) => {
        ...
        const t2 = st ? st.t2 : true
        const war = st ? st.war : true
        const merged = t1 && t2
        const row = [
          { key: "t1" as const, on: t1, gem: e.t1Gem, trait: e.t1 },
          { key: "t2" as const, on: t2, gem: e.t2Gem, trait: e.t2 },
          { key: "war" as const, on: war, gem: e.warGem, trait: e.war },
        ]
        return (
          <div key={e.hash} className="border-b py-2 text-sm last:border-b-0">
            <div className="mb-1 flex items-center gap-2 text-muted-foreground">
              <span className="tabular-nums">0x{e.hash}</span>
              <span>·</span>
              <span>{gemName(e.awakening)}</span>
            </div>
            <div className="flex flex-wrap items-center gap-x-6 gap-y-1">
              {row.map((r) => (
                <label key={r.key} className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs text-muted-foreground">({traitName(r.trait)})</span>
                </label>
              ))}
            </div>
            {merged && (
              <div className="mt-1 text-xs text-muted-foreground">
                {lang === "zh"
                  ? `t1+t2 同时启用，合并为「${gemName(e.awakening)}」（觉醒＋）`
                  : `t1+t2 enabled: merged as ${gemName(e.awakening)}`}
              </div>
            )}
          </div>
        )
