...
- `v0.8.0` 将扩展槽数量设置移入 ImGui，采用重启生效与缩减确认事务；预设继续保留完整 24 槽定义，并修正全部 29 个可选角色身份的哈希名称映射（古兰与姬塔分别计算）。
...
- 当前未发布修复将预设文档升级为 v4：每个角色的已选预设 ID 与预设内容一起持久化，关闭菜单、切换角色或重建 UI 后不再无条件回落到临时预设。
...
7. v4 预设文档在 `SelectedPresetIdsByCharacter` 中区分“键缺失”和“值为 null”：键缺失表示旧数据尚待按当前 24 槽恢复，null 表示用户当前明确处于临时预设。
8. 转让或删除前会先恢复来源角色的真实选中项；只有转让／删除当前选中预设才会把来源角色写成临时预设，操作非当前预设不会改动来源或目标角色的选择。
...
- v3 起每个预设只属于一个角色，槽位固定容量 24；同一预设中的非零物理槽去重。
- v4 把每个角色的选中预设 ID 纳入同一事务；新建／套用会选择对应预设，手工修改槽位会写入显式 null，转让或删除非当前预设保持原选择不变。
