...
$common = @(
    ...
    @{ Gem = '1E2EBC39'; T1 = '57AB5B10'; T2 = '318D12E9' }  # slot 7: 追击Ⅴ＋ + 迅捷能力Ⅴ＋ (Pursuit V+ + Swift Ability V+)
    @{ Gem = '49434696'; T1 = 'BF78FBFC'; T2 = '887AE0B0'; LV = 20 }  # slot 8: 可怕的漆黑钳蟹因子 Lv20 (event sigil; trait2 = 不选择)
)
...
foreach ($c in $chars) {
    ...
    [void]$sb.AppendLine("      TemplateGemSlot{0x$($common[5].Gem), 0x$($common[5].T1), 15, 0x$($common[5].T2), 15, 15}, // slot7 Pursuit V+ + Swift Ability V+")
    [void]$sb.AppendLine("      TemplateGemSlot{0x$($common[6].Gem), 0x$($common[6].T1), $($common[6].LV), 0x$($common[6].T2), 0, $($common[6].LV)}, // slot8 可怕的漆黑钳蟹因子 Lv$($common[6].LV) (event sigil)")
    [void]$sb.AppendLine('   }},')
}
[void]$sb.AppendLine('};')
