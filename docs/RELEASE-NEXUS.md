...
通过**运行时合成因子**自动给角色套用固定配装——不需要库存物品、不修改存档、没有界面、没有 F8。

[截图 1][截图 2]

## 功能

- 直接把**合成因子**注入本地状态计算的词条链路（与游戏 12 个本体槽同一套机制，额外扩展 5 个虚拟槽）；
- **零库存依赖**：不需要拥有、刷取或合成任何因子即可生效；
- **不写存档**、不改 `GemData.WORN_BY`，无任何持久化修改；
- 战斗生效由原生注入追踪验证（日志：`Live battle Trait contribution confirmed ...`）。
