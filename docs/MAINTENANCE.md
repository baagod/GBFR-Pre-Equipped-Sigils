...
### 未来方向（未做）
- **配置化优先（2026-09-04 决策）**：外置配装表 + 多预设/每角色开关——玩家自由预配，一次性终结"能不能加 xx/X 顶满"类需求（需求方从作者变为玩家）。
  落地草案：阶段1（v0.3.6）内置 2-3 套预设（默认/输出向等）+ 配置文件选预设（托管层读配置→native）；阶段2（v0.4.0+）词条级自由配置。注意：hash 引用需查表（§4）、无效条目 fail-closed、ABI 变更需三方同步（§7）。
