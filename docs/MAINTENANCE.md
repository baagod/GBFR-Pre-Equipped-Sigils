...
> 游戏版本 Granblue Fantasy: Relink Endless Ragnarok **2.0.5**；当前版本 **0.5.8**（ABI v17）；机制沿革见 §11。
...
- 主因子按 `name`（英文名）**分组**（同名变体一行，下拉只显示唯一名字）；`_74` 专属行的 name/zh 保留官方"＋"后缀，不影响分组。
...
- **版本** v0.5.8（ABI v17）。入口配装：每角色专属 3 独立槽（T1/T2/战气，默认全开）+ 玩家通用槽（固定 12 行编辑器，无内置通用默认）。
