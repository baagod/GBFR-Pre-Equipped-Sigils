...
struct CharacterExclusiveLoadout
{
   ...
   TemplateGemSlot awakening{};   // slot 0
...
constexpr CharacterExclusiveLoadout kCharacterExclusives[] = {
   { 0x079DF0CC, // character
      TemplateGemSlot{0x98A6D249, 0x151E4674, 15, 0xA374FDF0, 15, 15}, // slot0 awakening+ (2 exclusives)
      TemplateGemSlot{0x1791546F, 0xD76F4D24, 15, 0x887AE0B0, 0, 15}, // slot1 war spirit (single trait)
   },
   { 0x0D21B430, // character
      TemplateGemSlot{0x4F01D6CA, 0x6EBFA176, 15, 0xF1D5DBD0, 15, 15}, // slot0 awakening+ (2 exclusives)
      TemplateGemSlot{0x6674C639, 0x4F135217, 15, 0x887AE0B0, 0, 15}, // slot1 war spirit (single trait)
   },
   { 0x18E2F9F9, // character
      TemplateGemSlot{0x9ADA3E00, 0x3BFED918, 15, 0xF8496336, 15, 15}, // slot0 awakening+ (2 exclusives)
      TemplateGemSlot{0x621A2A97, 0x9AFDFA9E, 15, 0x887AE0B0, 0, 15}, // slot1 war spirit (single trait)
   },
   { 0x1BB37EF0, // character
      TemplateGemSlot{0x895ABBF6, 0x26956F25, 15, 0x1DE14C65, 15, 15}, // slot0 awakening+ (2 exclusives)
      TemplateGemSlot{0x41AC1082, 0xDBA19768, 15, 0x887AE0B0, 0, 15}, // slot1 war spirit (single trait)
   },
   { 0x22E437E5, // character
      TemplateGemSlot{0xE19B1965, 0x8CDF9382, 15, 0xD1012D8C, 15, 15}, // slot0 awakening+ (2 exclusives)
      TemplateGemSlot{0x8A3819C0, 0x6316CBEB, 15, 0x887AE0B0, 0, 15}, // slot1 war spirit (single trait)
   },
   { 0x25D46F4B, // character
      TemplateGemSlot{0xD8A464F1, 0x9ACE140B, 15, 0x7B5B081D, 15, 15}, // slot0 awakening+ (2 exclusives)
      TemplateGemSlot{0x6B920DA2, 0x79266456, 15, 0x887AE0B0, 0, 15}, // slot1 war spirit (single trait)
   },
   { 0x296471BE, // character
      TemplateGemSlot{0x6AAE4B8F, 0x77C809F5, 15, 0x9230E3F5, 15, 15}, // slot0 awakening+ (2 exclusives)
      TemplateGemSlot{0x67C1E5E3, 0x7B4FC47A, 15, 0x887AE0B0, 0, 15}, // slot1 war spirit (single trait)
   },
   { 0x2A26B1B2, // character
      TemplateGemSlot{0x52A6E299, 0xCD030268, 15, 0xA38510E2, 15, 15}, // slot0 awakening+ (2 exclusives)
      TemplateGemSlot{0x0713D928, 0xDADE14DC, 15, 0x887AE0B0, 0, 15}, // slot1 war spirit (single trait)
   },
   ...
   { 0x2EBE91D5, // character
      TemplateGemSlot{0x673C5D8F, 0x2E65A774, 15, 0x16EFF868, 15, 15}, // slot0 awakening+ (2 exclusives)
      TemplateGemSlot{0x3D8D9109, 0xD8F66C1C, 15, 0x887AE0B0, 0, 15}, // slot1 war spirit (single trait)
   },
   { 0x4D0A60C3, // character
      TemplateGemSlot{0xE2B380E5, 0xB48EEF48, 15, 0x11AAE5F5, 15, 15}, // slot0 awakening+ (2 exclusives)
      TemplateGemSlot{0x43F26A91, 0xC00163B3, 15, 0x887AE0B0, 0, 15}, // slot1 war spirit (single trait)
   },
   { 0x627BCB0D, // character
      TemplateGemSlot{0xAB835493, 0x86CBCDC4, 15, 0x05FA4599, 15, 15}, // slot0 awakening+ (2 exclusives)
      TemplateGemSlot{0x450B862C, 0xC7D379F1, 15, 0x887AE0B0, 0, 15}, // slot1 war spirit (single trait)
   },
   { 0x646C3168, // character
      TemplateGemSlot{0x5A360EA8, 0x30773197, 15, 0x47384248, 15, 15}, // slot0 awakening+ (2 exclusives)
      TemplateGemSlot{0x2D70C37D, 0x807B6684, 15, 0x887AE0B0, 0, 15}, // slot1 war spirit (single trait)
   },
   { 0x718E1A14, // character
      TemplateGemSlot{0xB8C44D5E, 0xD40D1E9B, 15, 0x15806DFC, 15, 15}, // slot0 awakening+ (2 exclusives)
      TemplateGemSlot{0x5D592FDD, 0x4E5F6706, 15, 0x887AE0B0, 0, 15}, // slot1 war spirit (single trait)
   },
   { 0x74DD4C79, // character
      TemplateGemSlot{0xA8A0CBFF, 0x06719232, 15, 0xED8D8AD8, 15, 15}, // slot0 awakening+ (2 exclusives)
      TemplateGemSlot{0x4119F09B, 0x5559232F, 15, 0x887AE0B0, 0, 15}, // slot1 war spirit (single trait)
   },
   { 0x978E4B18, // character
      TemplateGemSlot{0xCE16D68B, 0x5463232F, 15, 0x451D814C, 15, 15}, // slot0 awakening+ (2 exclusives)
      TemplateGemSlot{0x3069C2FE, 0x0F026CF0, 15, 0x887AE0B0, 0, 15}, // slot1 war spirit (single trait)
   },
   { 0x9A8AF295, // character
      TemplateGemSlot{0x95CC3CB8, 0xD176D262, 15, 0x461A8E07, 15, 15}, // slot0 awakening+ (2 exclusives)
      TemplateGemSlot{0x51E98A7C, 0xB953CC1E, 15, 0x887AE0B0, 0, 15}, // slot1 war spirit (single trait)
   },
   { 0x9B15CFB1, // character
      TemplateGemSlot{0x23953FD4, 0x7D75D904, 15, 0xBE3404B9, 15, 15}, // slot0 awakening+ (2 exclusives)
      TemplateGemSlot{0x61A1A299, 0x3EB345D7, 15, 0x887AE0B0, 0, 15}, // slot1 war spirit (single trait)
   },
   { 0xA3A3CB2F, // character
      TemplateGemSlot{0xAF8E7E7E, 0x93A2093C, 15, 0x7AD0C010, 15, 15}, // slot0 awakening+ (2 exclusives)
      TemplateGemSlot{0x98E9E6EF, 0xB064A634, 15, 0x887AE0B0, 0, 15}, // slot1 war spirit (single trait)
   },
   { 0xAA66178A, // character
      TemplateGemSlot{0x02B1F8C0, 0xEC3CF174, 15, 0xAF513A9D, 15, 15}, // slot0 awakening+ (2 exclusives)
      TemplateGemSlot{0x5FB842E4, 0xE6B92E34, 15, 0x887AE0B0, 0, 15}, // slot1 war spirit (single trait)
   },
   { 0xBAD16E3B, // character
      TemplateGemSlot{0x8ECBB0A3, 0xE85FF8E0, 15, 0x8572B8AF, 15, 15}, // slot0 awakening+ (2 exclusives)
      TemplateGemSlot{0xAD8CAEFB, 0x81B293D9, 15, 0x887AE0B0, 0, 15}, // slot1 war spirit (single trait)
   },
   { 0xBDEF7181, // character
      TemplateGemSlot{0x02472C43, 0xE60A735C, 15, 0x6FF05223, 15, 15}, // slot0 awakening+ (2 exclusives)
      TemplateGemSlot{0x4CDCE25B, 0xBA504607, 15, 0x887AE0B0, 0, 15}, // slot1 war spirit (single trait)
   },
   { 0xC3FFD418, // character
      TemplateGemSlot{0xB441275D, 0xD908223D, 15, 0x7351D602, 15, 15}, // slot0 awakening+ (2 exclusives)
      TemplateGemSlot{0x40738CDD, 0xA339D642, 15, 0x887AE0B0, 0, 15}, // slot1 war spirit (single trait)
   },
   { 0xC8616284, // character
      TemplateGemSlot{0x9BD1CC24, 0x23D0F67F, 15, 0xC2A4C7A9, 15, 15}, // slot0 awakening+ (2 exclusives)
      TemplateGemSlot{0x515E693C, 0x8519AD4A, 15, 0x887AE0B0, 0, 15}, // slot1 war spirit (single trait)
   },
   { 0xDD7A151E, // character
      TemplateGemSlot{0x1BBE919C, 0xAA83F548, 15, 0x921B6B0C, 15, 15}, // slot0 awakening+ (2 exclusives)
      TemplateGemSlot{0x7C7EC053, 0x0E42BE1B, 15, 0x887AE0B0, 0, 15}, // slot1 war spirit (single trait)
   },
   { 0xE7053919, // character
      TemplateGemSlot{0x1A57AEF1, 0x29B07BEB, 15, 0xA63B89CD, 15, 15}, // slot0 awakening+ (2 exclusives)
      TemplateGemSlot{0xCEF31894, 0xFDD1AD24, 15, 0x887AE0B0, 0, 15}, // slot1 war spirit (single trait)
   },
   { 0xF0EB77EF, // character
      TemplateGemSlot{0xE4F986D9, 0x7440E869, 15, 0xCD124165, 15, 15}, // slot0 awakening+ (2 exclusives)
      TemplateGemSlot{0x590C19C8, 0xD7F9BB88, 15, 0x887AE0B0, 0, 15}, // slot1 war spirit (single trait)
   },
   { 0xFC6CDF7B, // character
      TemplateGemSlot{0x119B24A8, 0x0CD6C625, 15, 0xA3B49220, 15, 15}, // slot0 awakening+ (2 exclusives)
      TemplateGemSlot{0x76D4716B, 0xDAEFBB27, 15, 0x887AE0B0, 0, 15}, // slot1 war spirit (single trait)
   },
   { 0xFD3BE362, // character
      TemplateGemSlot{0xAEEF8343, 0x9A9DC170, 15, 0x522E2388, 15, 15}, // slot0 awakening+ (2 exclusives)
      TemplateGemSlot{0x34C77091, 0xB85202BC, 15, 0x887AE0B0, 0, 15}, // slot1 war spirit (single trait)
   },
};

// General slots 3-9 (identical for every character):
//   slot3 激昂V＋ (single trait; moved from the war-spirit slot)
//   slot4 豪胆V＋ + 自动复活, slot5 不动V＋ + 明镜止水, slot6 刚健V＋ + 药水携带数,
//   slot7 守护V＋ + 躲避性能, slot8 追击V＋ + 迅捷能力,
//   slot9 可怕的漆黑钳蟹因子 Lv20 (single trait, trait2 = not-selected sentinel).
constexpr std::array<TemplateGemSlot, 7> kGeneralSlots = {
   TemplateGemSlot{0x04AC2281, 0xB5FF9FD3, 15, 0x887AE0B0, 0, 15}, // slot3 激昂V＋ (single)
   TemplateGemSlot{0x335DA2A5, 0xE69A4694, 15, 0x95F3FA86, 15, 15}, // slot4 豪胆V＋
   TemplateGemSlot{0xB1CCC211, 0xB6E31F76, 15, 0xD2C8E10A, 15, 15}, // slot5 不动V＋
   TemplateGemSlot{0x297D03F7, 0x74AA75D6, 15, 0x24883AF3, 15, 15}, // slot6 刚健V＋
   TemplateGemSlot{0x35637B96, 0xE0ABFDFE, 15, 0x8B3BF60C, 15, 15}, // slot7 守护V＋
   TemplateGemSlot{0x1E2EBC39, 0x57AB5B10, 15, 0x318D12E9, 15, 15}, // slot8 追击V＋
   TemplateGemSlot{0x49434696, 0xBF78FBFC, 20, 0x887AE0B0, 0, 20} // slot9 钳蟹 Lv20
};

CharacterTemplate BuildCharacterTemplate(
   const CharacterExclusiveLoadout& exclusive) noexcept
{
   CharacterTemplate character{};
   character.character_hash = exclusive.character_hash;
   character.slots[0] = exclusive.awakening;
   character.slots[1] = exclusive.war_spirit;
   for (size_t index = 0; index < kGeneralSlots.size(); ++index)
      character.slots[2 + index] = kGeneralSlots[index];
   return character;
}
...
void InitializeRuntimeTemplates()
{
   ...
   for (const CharacterExclusiveLoadout& entry : kCharacterExclusives)
   {
      if (index >= g_runtime_templates.size())
         break;
      g_runtime_templates[index] = BuildCharacterTemplate(entry);
      ...
      for (CharacterTemplate& character : g_runtime_templates)
      {
         ...
         if (use_builtin)
         {
            if (builtin_index < kBuiltinTemplateCount)
               character = BuildCharacterTemplate(kCharacterExclusives[builtin_index++]);
            else
